...
```bash
ls "$SFI_PATH/sf-tutorial/artifacts/packs/review-loop-rig/"
cat "$SFI_PATH/sf-tutorial/artifacts/packs/review-loop-rig/pack.toml"
cat "$SFI_PATH/sf-tutorial/artifacts/packs/review-loop-rig/formulas/mol-refinery-review-loop-patrol.formula.toml"
```
...
rm -rf "$SFI_PATH/factory1/packs/review-loop-rig"

cp -r "$SFI_PATH/sf-tutorial/artifacts/packs/review-loop-rig" \
      "$SFI_PATH/factory1/packs/review-loop-rig"
```
...
```bash
find "$SFI_PATH/factory1/packs/review-loop-rig" -name pack.toml
```

**Expected output**

```text
$SFI_PATH/factory1/packs/review-loop-rig/pack.toml
```
...
```bash
# 3. Worktrees cleaned up — only the main worktree remains in $SFI_PATH/ascii-art
git worktree list
```
