...
```text
bead-gate-rig (no architect-rig row).
```
...
```text
all three rows. mol-architect-review and mol-polecat-pr
...
```text
subject "Bead blocked at project-manager: <bead-id>"
```
...
The `mol-bead-review` formula is loaded.
...
The cleared `h.md` bead carries a PASS verdict.
...
```text
metadata.bead_review_passed=true and a "project-manager: PASSED..."
note. After the polecat/refinery/architect chain runs, pr_url + pr_number
populated, file merged.
```

The deliberately-broken bead is blocked.
