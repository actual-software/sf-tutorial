...
« [previous: 04 Architect agent](./04-adr-reviewer.md) | [next: 05.2 Bead gate checks — Grill Me](./05.2-bead-gate-checks.md) »
...
- [Walkthrough](#walkthrough)
  ...
  - [3. Sling the bead at the project-manager (new entry point)](#3-sling-the-bead-at-the-project-manager-new-entry-point)
  - [4. Watch the project-manager run and the polecat pick up](#4-watch-the-project-manager-run-and-the-polecat-pick-up)
  - [5. Demonstrate the FAIL path](#5-demonstrate-the-fail-path)
  - [6. Unblock the bead by hand and re-sling](#6-unblock-the-bead-by-hand-and-re-sling)
  - [7. Reflect](#7-reflect)
...
By the end of this exercise you will have installed a `bead-gate-rig` pack that puts a `project-manager` agent at the front of the factory, slung a clean bead through it to the polecat pool, and watched it block a deliberately malformed bead with feedback.
...
# Expect: bead-gate-rig (no architect-rig row).
...
```bash
gc formula list | grep -E "mol-bead-review|mol-architect-review|mol-polecat-pr"
# Expect: all three rows. mol-architect-review and mol-polecat-pr
# resolve transitively via architect-rig and pr-gate-rig.
```
...
### 3. Sling the bead at the project-manager (new entry point)
...
### 4. Watch the project-manager run and the polecat pick up
...
```bash
gc session list        # confirm project-manager is running
```
...
```bash
gc mail inbox mayor
# Expect: subject "Bead blocked at project-manager: <bead-id>"
```

### 6. Unblock the bead by hand and re-sling
...
```bash
# 1. The mol-bead-review formula is loaded.
gc formula list | grep mol-bead-review
# Expect: one row.

# 2. The cleared h.md bead carries a PASS verdict.
gc bd show $BEAD_ID
# Expect: metadata.bead_review_passed=true and a "project-manager: PASSED..."
# note. After the polecat/refinery/architect chain runs, pr_url + pr_number
# populated, file merged.

# 3. The deliberately-broken bead is blocked.
gc bd show $BAD_BEAD
# After step 5: status=blocked, bead_review_passed=false,
# bead_review_feedback set. After step 6: status=open,
# bead_review_passed=true.

# 4. New letter on origin/main.
cd $ASCII_ART_PATH
