...
**Workshop W-4 · Review Loops · Wednesday 11:00–12:00 · 60 minutes · one shared example**

## Block W-4: Review Loops

This page opens W-4, and the block is three pages read as one: this one, [03](./03-branch-protection.md) and [04](./04-adr-reviewer.md). Everything under this heading frames all three. This page's own walkthrough starts at [Objective](#objective).
...
- [W-3](./00.1-setup-foundation.md) complete: `factory1` running, `ascii-art` registered, at least one letter merged through a pull request.
...
Read this before you open the walkthrough, because the three pages in this block are much easier to hold in your head as answers to three questions than as three unrelated features.
...
That is a governance posture, and it is the thing most teams cannot currently demonstrate about their agent setups. This hour is where the factory earns it. This page answers the first, `03` answers the second, and `04` is the shape of an automated check that produces real evidence rather than a rubber stamp.
...
| --- | --- | --- |
| 11:00–11:20 | This page, first review loop | A required round of feedback before any bead becomes a pull request |
| 11:20–11:40 | [`03` Branch protection](./03-branch-protection.md) | Merge to `main` is human-only, and a direct push is rejected in front of you |
| 11:40–12:00 | [`04` Architect agent](./04-adr-reviewer.md) | Every bead reviewed against the rig's ADRs before the merge gate runs |
...
In `04`, the optional step 8 slings a bead the architect will reject. Do it if you have the minutes. Watching a reviewer produce a reasoned verdict against a document you can open is the difference between "the agent said no" and "the agent cited the ADR". That distinction is what the third governance requirement is about.
...
Write down which of the three you would have the hardest time explaining to your own reviewers. That is a useful thing to carry into [L-1](./07-plan-your-factory.md) this afternoon.
...
The question worth sitting with, and worth bringing to [L-2](../hardening/02-specialize-reviewers-per-domain.md) tomorrow: on your own repo, which changes deserve four reviewers and which deserve none? A factory that reviews everything equally is a factory that is either too slow or too shallow.

### Block troubleshooting

- **The protection script says the branch does not exist.** `main` has to be pushed before it can be protected. Finish [`00.2`](./00.2-setup-foundation.md)'s push step first.
...
Review catches bad work on the way out. [W-5 Requirement Gates](./05.1-bead-gate-checks.md) catches underspecified work on the way in, which is cheaper.

---

The rest of this page is the first of the block's three pages.
